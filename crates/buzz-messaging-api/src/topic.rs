...
//!
//! The wire names (`buzz:{community}:channel:{uuid}`, `buzz:{community}:global`)
//! were born as Redis pub/sub channel names and are kept verbatim by every
//! transport backend — ZeroMQ prefix subscriptions match the same strings —
//! so rolling deployments can mix backends without a topic migration.

use buzz_core::{CommunityId, TenantContext};
use thiserror::Error;
use uuid::Uuid;

/// Key prefix for Buzz-scoped pub/sub topics and keys.
pub const BUZZ_PREFIX: &str = "buzz";

/// A malformed wire topic name.
#[derive(Debug, Error)]
...
impl EventTopicKey {
    ...
    /// Wire topic name for this key (Redis pub/sub channel name compatible).
    ...
    /// Parse a wire topic name into a scoped event topic.
    pub fn parse_redis_channel(channel: &str) -> Result<Self, TopicError> {
        let mut parts = channel.split(':');
        let Some(prefix) = parts.next() else {
            return Err(TopicError(channel.to_string()));
        };
        ...
        let community_id = Uuid::parse_str(community)
            ...
            .map_err(|_| TopicError(channel.to_string()))?;
        ...
        let topic = match scope {
            ...
            "channel" => {
                ...
                EventTopic::Channel(
                    Uuid::parse_str(channel_id).map_err(|_| TopicError(channel.to_string()))?,
                )
            }
            _ => return Err(TopicError(channel.to_string())),
        };
...
/// Wire topic for exact-channel events under `ctx`.
...
/// Wire topic for community-global events under `ctx`.
