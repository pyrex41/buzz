...
/// Database handle. Clone is cheap (Arc-backed pools).
#[derive(Clone, Debug)]
pub struct Db {
    backend: DbBackend,
    /// Maximum connections configured for the writer pool.
    pub(crate) max_connections: u32,
    /// Freshness fence gating cursor-page routing to the replica.
    ///
    /// Starts closed; on the Postgres backend a background probe
    /// ([`replica_fence::run_probe`]) advances it after each verified
    /// writer→replica LSN handshake. On SQLite there is no replica, the
    /// probe never spawns, and the fence stays closed — every cursor page
    /// routes to the (only) pool, which is exactly the degraded-but-correct
    /// no-replica behavior.
    pub(crate) fence: std::sync::Arc<replica_fence::ReplicaFence>,
}

/// The storage engine behind a [`Db`] handle (Hive plan Phase 2).
///
/// `Db` keeps one public surface; each method dispatches internally.
/// Methods without a SQLite arm yet return
/// [`DbError::UnsupportedBackend`] — the relay's Solo-profile config
/// gates keep those paths unreached.
#[derive(Clone, Debug)]
enum DbBackend {
    Pg {
        pool: PgPool,
        /// Optional read-replica pool; `None` routes all reads to the writer.
...
impl Db {
    ...
    pub async fn new(config: &DbConfig) -> Result<Self> {
        ...
        Ok(Self {
            backend: DbBackend::Pg { pool, read_pool },
            max_connections: config.max_connections,
            fence: std::sync::Arc::new(replica_fence::ReplicaFence::new()),
        })
    }

    /// Open (or create) a SQLite database for the single-node Solo profile.
    ///
    /// WAL journaling, foreign keys on, NORMAL synchronous, and a busy
    /// timeout: SQLite's single-writer semantics replace the Postgres
    /// advisory-lock machinery (ADR 0001).
    ...
    /// Writer pool of the Postgres backend.
    ///
    /// Returns [`DbError::UnsupportedBackend`] on SQLite — the marker that a
    /// method has no SQLite arm yet (or is inherently Postgres-only).
    pub(crate) fn pg(&self) -> Result<&PgPool> {
        match &self.backend {
            DbBackend::Pg { pool, .. } => Ok(pool),
            DbBackend::Sqlite { .. } => Err(DbError::UnsupportedBackend(
                "operation not yet supported on the sqlite backend",
            )),
        }
    }

    /// Lag-tolerant read pool of the Postgres backend: the replica when
    /// configured, otherwise the writer (see the routing contract on the
    /// former `Db::read`).
    pub(crate) fn pg_read(&self) -> Result<&PgPool> {
        match &self.backend {
            DbBackend::Pg { pool, read_pool } => Ok(read_pool.as_ref().unwrap_or(pool)),
            DbBackend::Sqlite { .. } => Err(DbError::UnsupportedBackend(
                "operation not yet supported on the sqlite backend",
            )),
        }
    ...
    ///
    /// Composition-root escape hatch: lets the relay wire backend-matched
    /// sidecar services that hold their own connection handle (the FTS5
    /// search service in `buzz-search`, `AuditService::new_sqlite` in
    /// `buzz-audit`) over the same database file, without threading `Db`
    ...
    pub(crate) fn sqlite(&self) -> Result<&sqlx::SqlitePool> {
        match &self.backend {
            ...
            DbBackend::Pg { .. } => Err(DbError::UnsupportedBackend(
                "operation requires the sqlite backend",
            )),
        }
    }
    ...
    pub async fn spawn_fence_probe(&self) -> Result<bool> {
        let DbBackend::Pg { pool, read_pool } = &self.backend else {
            return Ok(false);
        };
        let Some(read_pool) = read_pool else {
            return Ok(false);
        };
        replica_fence::verify_floor_guard_catalog(pool).await?;
        replica_fence::verify_floor_guard_behavior(pool).await?;
    ...
    /// Run pending database migrations for the active backend.
    pub async fn migrate(&self) -> Result<()> {
        match &self.backend {
            DbBackend::Pg { pool, .. } => migration::run_migrations(pool).await,
            DbBackend::Sqlite { pool } => sqlite::run_migrations(pool).await,
        }
    }
    ...
    pub async fn ping(&self) -> bool {
        match &self.backend {
            DbBackend::Pg { pool, .. } => sqlx::query("SELECT 1").execute(pool).await.is_ok(),
    ...
    pub fn pool_stats(&self) -> DbPoolStats {
        match &self.backend {
            DbBackend::Pg { pool, .. } => DbPoolStats {
                size: pool.size(),
                idle: pool.num_idle() as u32,
                max: self.max_connections,
            },
            DbBackend::Sqlite { pool } => DbPoolStats {
                size: pool.size(),
                idle: pool.num_idle() as u32,
                max: self.max_connections,
            },
    ...
    pub async fn try_lock_usage_metrics(
        ...
        let mut connection = self.pg()?.acquire().await?;
    ...
    pub async fn admin_list_reports(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::admin_moderation::admin_list_reports(
                pool,
                community_id,
                status,
                report_type,
    ...
    pub async fn admin_get_report(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::admin_moderation::admin_get_report(pool, id).await;
        }
        admin_moderation::get_report(self.pg()?, id).await
    }
    ...
    pub async fn admin_list_feedback(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::admin_moderation::admin_list_feedback(pool, limit).await;
        }
        admin_moderation::list_feedback(self.pg()?, limit).await
    }
    ...
    pub async fn admin_get_feedback(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::admin_moderation::admin_get_feedback(pool, id).await;
        }
        admin_moderation::get_feedback(self.pg()?, id).await
    }

    /// Return total number of communities on this relay.
    pub async fn usage_community_count(&self) -> Result<i64> {
        usage::community_count(self.pg()?).await
    }
    ...
    pub async fn usage_relay_member_counts(&self) -> Result<Vec<usage::CommunityMemberCount>> {
        usage::relay_member_counts(self.pg()?).await
    }
    ...
    pub async fn usage_active_user_counts(
        ...
        usage::active_user_counts(self.pg()?, interval_sql).await
    }
    ...
    pub async fn begin_transaction(&self) -> Result<sqlx::Transaction<'static, sqlx::Postgres>> {
        self.pg()?.begin().await.map_err(Into::into)
    }
    ...
    pub async fn claim_due_push_match_batch(
        ...
        push::claim_due_match_batch(self.pg()?, limit, lease_until).await
    }
    ...
    pub async fn active_push_match_leases(
        ...
        push::active_match_leases(self.pg()?, community).await
    }
    ...
    pub async fn complete_push_match_batch(
        ...
        push::complete_match_batch(self.pg()?, community, claim_id, event_ids).await
    }
    ...
    pub async fn enqueue_push_wake(
        ...
        push::enqueue_wake(self.pg()?, community, author, installation_id, wake).await
    }
    ...
    pub async fn enqueue_push_wakes(
        ...
        push::enqueue_wakes(self.pg()?, community, requests).await
    }
    ...
    pub async fn claim_due_push_wakes(
        ...
        push::claim_due_wakes(self.pg()?, community, limit, lease_until).await
    }
    ...
    pub async fn revalidate_push_wake(
        ...
        push::revalidate_wake_for_send(self.pg()?, community, id, claim_id).await
    }
    ...
    pub async fn complete_push_wake(
        ...
        push::complete_wake(self.pg()?, community, id, claim_id).await
    }
    ...
    pub async fn retry_push_wake(
        ...
        push::retry_wake(self.pg()?, community, id, claim_id, next).await
    }
    ...
    pub async fn fail_push_wake(
        ...
        push::fail_wake(self.pg()?, community, id, claim_id).await
    }
    ...
    pub async fn membership_pairs(
        ...
        channel::membership_pairs(self.pg()?, community_id, channel_ids, pubkeys).await
    }
    ...
    pub async fn get_bot_members(
        ...
        channel::get_bot_members(self.pg()?, community_id).await
    }
    ...
    pub async fn get_member_count(
        ...
        channel::get_member_count(self.pg()?, community_id, channel_id).await
    }
    ...
    pub async fn get_member_counts_bulk(
        ...
        channel::get_member_counts_bulk(self.pg()?, community_id, channel_ids).await
    }
    ...
    pub async fn ensure_user(&self, community_id: CommunityId, pubkey: &[u8]) -> Result<bool> {
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::ensure_user(pool, community_id, pubkey).await;
        }
        user::ensure_user(self.pg()?, community_id, pubkey).await
    }
    ...
    pub async fn get_user(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::get_user(pool, community_id, pubkey).await;
        }
        user::get_user(self.pg()?, community_id, pubkey).await
    }
    ...
    pub async fn update_user_profile(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::update_user_profile(
                pool,
                community_id,
                pubkey,
                display_name,
    ...
    pub async fn get_user_by_nip05(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::get_user_by_nip05(pool, community_id, local_part, domain).await;
        }
        user::get_user_by_nip05(self.pg()?, community_id, local_part, domain).await
    }
    ...
    pub async fn search_users(
        ...
        user::search_users(self.pg()?, community_id, query, limit).await
    }
    ...
    pub async fn set_agent_owner(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::set_agent_owner(pool, community_id, agent_pubkey, owner_pubkey)
                .await;
        }
        user::set_agent_owner(self.pg()?, community_id, agent_pubkey, owner_pubkey).await
    }
    ...
    pub async fn get_agent_channel_policy(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::get_agent_channel_policy(pool, community_id, pubkey).await;
        }
        user::get_agent_channel_policy(self.pg()?, community_id, pubkey).await
    }
    ...
    pub async fn is_agent_owner(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::is_agent_owner(pool, community_id, target_pubkey, actor_pubkey)
                .await;
        }
        user::is_agent_owner(self.pg()?, community_id, target_pubkey, actor_pubkey).await
    }
    ...
    pub async fn set_channel_add_policy(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::user::set_channel_add_policy(pool, community_id, pubkey, policy).await;
        }
        user::set_channel_add_policy(self.pg()?, community_id, pubkey, policy).await
    }
    ...
    pub async fn find_dm_by_participants(
        ...
        dm::find_dm_by_participants(self.pg()?, community_id, participant_hash).await
    }
    ...
    pub async fn create_dm(
        ...
        dm::create_dm(self.pg()?, community_id, participants, created_by).await
    }
    ...
    pub async fn list_dms_for_user(
        ...
        dm::list_dms_for_user(self.pg()?, community_id, pubkey, limit, cursor).await
    }
    ...
    pub async fn open_dm(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::dm::open_dm(pool, community_id, pubkeys, created_by).await;
        }
        dm::open_dm(self.pg()?, community_id, pubkeys, created_by).await
    }
    ...
    pub async fn hide_dm(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::dm::hide_dm(pool, community_id, channel_id, pubkey).await;
        }
        dm::hide_dm(self.pg()?, community_id, channel_id, pubkey).await
    }
    ...
    pub async fn unhide_dm(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::dm::unhide_dm(pool, community_id, channel_id, pubkey).await;
        }
        dm::unhide_dm(self.pg()?, community_id, channel_id, pubkey).await
    }
    ...
    pub async fn list_hidden_dms(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::dm::list_hidden_dms(pool, community_id, pubkey).await;
        }
        dm::list_hidden_dms(self.pg()?, community_id, pubkey).await
    }
    ...
    pub async fn get_thread_replies(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::thread::get_thread_replies(
                pool,
                community_id,
                root_event_id,
                depth_limit,
    ...
    pub async fn get_thread_summary(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::thread::get_thread_summary(pool, community_id, event_id).await;
        }
        thread::get_thread_summary(self.pg()?, community_id, event_id).await
    }
    ...
    pub async fn get_channel_window(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::thread::get_channel_window(
                pool,
                community_id,
                channel_id,
                limit,
        ...
        let pool = match &cursor {
            Some((ts, _)) if self.has_read_pool() && self.fence.covers(*ts) => self.pg_read()?,
            _ => self.pg()?,
        };
    ...
    pub async fn get_thread_metadata_by_event(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::thread::get_thread_metadata_by_event(pool, community_id, event_id)
                .await;
        }
        thread::get_thread_metadata_by_event(self.pg()?, community_id, event_id).await
    }
    ...
    pub async fn decrement_reply_count(
        ...
        thread::decrement_reply_count(self.pg()?, community_id, parent_event_id, root_event_id)
            .await
    }
    ...
    pub async fn remove_reaction(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::reaction::remove_reaction(
                pool,
                community,
                event_id,
                event_created_at,
    ...
    pub async fn remove_reaction_by_source_event_id(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::reaction::remove_reaction_by_source_event_id(
                pool,
                community,
                reaction_event_id,
            )
        ...
        reaction::remove_reaction_by_source_event_id(self.pg()?, community, reaction_event_id).await
    }
    ...
    pub async fn get_reactions_bulk(
        ...
        reaction::get_reactions_bulk(self.pg()?, community, event_ids).await
    }
    ...
    pub async fn query_feed_mentions(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::feed::query_feed_mentions(
                pool,
                community,
                pubkey_bytes,
    ...
    pub async fn query_feed_needs_action(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::feed::query_feed_needs_action(
                pool,
                community,
                pubkey_bytes,
                accessible_channel_ids,
    ...
    pub async fn query_feed_activity(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::feed::query_feed_activity(
                pool,
                community,
                accessible_channel_ids,
                since,
        ...
        feed::query_activity(self.pg()?, community, accessible_channel_ids, since, limit).await
    }
    ...
    pub async fn list_tokens_by_owner(
        ...
        api_token::list_tokens_by_owner(self.pg()?, *community_id.as_uuid(), pubkey).await
    }
    ...
    pub async fn upsert_workflow(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::upsert_workflow(
                pool,
                community_id,
                id,
                channel_id,
    ...
    pub async fn get_workflow(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::get_workflow(pool, community_id, id).await;
        }
        workflow::get_workflow(self.pg()?, community_id, id).await
    }
    ...
    pub async fn list_channel_workflows(
        ...
        workflow::list_channel_workflows(self.pg()?, community_id, channel_id, limit, offset).await
    }
    ...
    pub async fn list_enabled_channel_workflows(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::list_enabled_channel_workflows(
                pool,
                community_id,
                channel_id,
            )
        ...
        workflow::list_enabled_channel_workflows(self.pg()?, community_id, channel_id).await
    }
    ...
    pub async fn list_all_enabled_workflows(&self) -> Result<Vec<workflow::WorkflowRecord>> {
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::list_all_enabled_workflows(pool).await;
        }
        workflow::list_all_enabled_workflows(self.pg()?).await
    }
    ...
    pub async fn claim_scheduled_workflow_fire(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::claim_scheduled_workflow_fire(
                pool,
                community_id,
                workflow_id,
                scheduled_for,
    ...
    pub async fn latest_scheduled_workflow_fire(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::latest_scheduled_workflow_fire(
                pool,
                community_id,
                workflow_id,
            )
        ...
        workflow::latest_scheduled_workflow_fire(self.pg()?, community_id, workflow_id).await
    }
    ...
    pub async fn attach_scheduled_workflow_run(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::attach_scheduled_workflow_run(
                pool,
                community_id,
                workflow_id,
                scheduled_for,
    ...
    pub async fn prune_scheduled_workflow_fires_before(
        ...
        workflow::prune_scheduled_workflow_fires_before(self.pg()?, older_than).await
    }
    ...
    pub async fn update_workflow_status(
        ...
        workflow::update_workflow_status(self.pg()?, community_id, id, status).await
    }
    ...
    pub async fn set_workflow_enabled(
        ...
        workflow::set_workflow_enabled(self.pg()?, community_id, id, enabled).await
    }

    /// Delete a workflow and all its runs/approvals.
    pub async fn delete_workflow(&self, community_id: CommunityId, id: Uuid) -> Result<()> {
        workflow::delete_workflow(self.pg()?, community_id, id).await
    }
    ...
    pub async fn delete_workflow_for_owner(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::delete_workflow_for_owner(
                pool,
                community_id,
                id,
                owner_pubkey,
        ...
        workflow::delete_workflow_for_owner(self.pg()?, community_id, id, owner_pubkey).await
    }
    ...
    pub async fn find_workflow_by_owner_and_name(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::find_workflow_by_owner_and_name(
                pool,
                community_id,
                owner_pubkey,
                name,
        ...
        workflow::find_by_owner_and_name(self.pg()?, community_id, owner_pubkey, name).await
    }
    ...
    pub async fn create_workflow_run(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::create_workflow_run(
                pool,
                community_id,
                workflow_id,
                trigger_event_id,
    ...
    pub async fn get_workflow_run(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::get_workflow_run(pool, community_id, id).await;
        }
        workflow::get_workflow_run(self.pg()?, community_id, id).await
    }
    ...
    pub async fn list_workflow_runs(
        ...
        workflow::list_workflow_runs(self.pg()?, community_id, workflow_id, limit).await
    }
    ...
    pub async fn update_workflow_run(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::update_workflow_run(
                pool,
                community_id,
                id,
                status,
    ...
    pub async fn create_approval(&self, params: workflow::CreateApprovalParams<'_>) -> Result<()> {
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::create_approval(pool, params).await;
        }
        workflow::create_approval(self.pg()?, params).await
    }
    ...
    pub async fn get_approval(
        ...
        workflow::get_approval(self.pg()?, community_id, token).await
    }
    ...
    pub async fn get_approval_by_stored_hash(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::get_approval_by_stored_hash(pool, community_id, token_hash)
                .await;
        }
        workflow::get_approval_by_stored_hash(self.pg()?, community_id, token_hash).await
    }
    ...
    pub async fn get_run_approvals(
        ...
        workflow::get_run_approvals(self.pg()?, community_id, workflow_id, run_id).await
    }
    ...
    pub async fn update_approval_by_stored_hash(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::workflow::update_approval_by_stored_hash(
                pool,
                community_id,
                token_hash,
                status,
    ...
    pub async fn ensure_future_partitions(&self, months_ahead: u32) -> Result<()> {
        partition::ensure_future_partitions(self.pg()?, months_ahead).await
    }
    ...
    pub async fn insert_product_feedback(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::product_feedback::insert_product_feedback(pool, community, feedback)
                .await;
        }
        product_feedback::insert(self.pg()?, community, feedback).await
    }
    ...
    pub async fn list_product_feedback(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::product_feedback::list_product_feedback(pool, limit).await;
        }
        product_feedback::list(self.pg()?, limit).await
    }
    ...
    pub async fn insert_moderation_report(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::insert_moderation_report(pool, community, report).await;
        }
        moderation::insert_report(self.pg()?, community, report).await
    }
    ...
    pub async fn list_moderation_reports(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::list_moderation_reports(pool, community, status, limit)
                .await;
        }
        moderation::list_reports(self.pg()?, community, status, limit).await
    }
    ...
    pub async fn get_moderation_report(
        ...
        moderation::get_report(self.pg()?, community, report_id).await
    }
    ...
    pub async fn get_moderation_report_by_event(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::get_moderation_report_by_event(
                pool,
                community,
                report_event_id,
            )
        ...
        moderation::get_report_by_event(self.pg()?, community, report_event_id).await
    }
    ...
    pub async fn resolve_moderation_report(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::resolve_moderation_report(
                pool,
                community,
                report_id,
                status,
    ...
    pub async fn ban_community_member(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::ban_community_member(
                pool, community, pubkey, actor, reason, expires_at,
            )
            .await;
        }
        moderation::ban_member(self.pg()?, community, pubkey, actor, reason, expires_at).await
    }
    ...
    pub async fn unban_community_member(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::unban_community_member(pool, community, pubkey, actor)
                .await;
        }
        moderation::unban_member(self.pg()?, community, pubkey, actor).await
    }
    ...
    pub async fn timeout_community_member(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::timeout_community_member(
                pool,
                community,
                pubkey,
                actor,
        ...
        moderation::timeout_member(self.pg()?, community, pubkey, actor, muted_until, reason).await
    }
    ...
    pub async fn untimeout_community_member(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::untimeout_community_member(pool, community, pubkey, actor)
                .await;
        }
        moderation::untimeout_member(self.pg()?, community, pubkey, actor).await
    }
    ...
    pub async fn moderation_restriction_state(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::moderation_restriction_state(pool, community, pubkey).await;
        }
        moderation::restriction_state(self.pg()?, community, pubkey).await
    }
    ...
    pub async fn get_community_ban(
        ...
        moderation::get_ban(self.pg()?, community, pubkey).await
    }
    ...
    pub async fn list_community_restrictions(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::list_community_restrictions(pool, community).await;
        }
        moderation::list_restricted(self.pg()?, community).await
    }
    ...
    pub async fn insert_moderation_action(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::insert_moderation_action(pool, community, action).await;
        }
        moderation::insert_action(self.pg()?, community, action).await
    }
    ...
    pub async fn list_moderation_actions(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::moderation::list_moderation_actions(pool, community, limit).await;
        }
        moderation::list_actions(self.pg()?, community, limit).await
    }
    ...
    pub async fn repo_name_owner(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::git_repo::repo_name_owner(pool, community, repo_id).await;
        }
        git_repo::repo_name_owner(self.pg()?, community, repo_id).await
    }
    ...
    pub async fn reserve_repo_name(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::git_repo::reserve_repo_name(pool, community, repo_id, owner_pubkey)
                .await;
        }
        git_repo::reserve_repo_name(self.pg()?, community, repo_id, owner_pubkey).await
    }
    ...
    pub async fn count_repos_for_owner(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::git_repo::count_repos_for_owner(pool, community, owner_pubkey).await;
        }
        git_repo::count_repos_for_owner(self.pg()?, community, owner_pubkey).await
    }
    ...
    pub async fn release_repo_name(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::git_repo::release_repo_name(pool, community, repo_id, owner_pubkey)
                .await;
        }
        git_repo::release_repo_name(self.pg()?, community, repo_id, owner_pubkey).await
    }
    ...
    pub async fn is_archived(&self, community_id: CommunityId, pubkey: &str) -> Result<bool> {
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::archived_identities::is_archived(pool, community_id, pubkey).await;
        }
        archived_identities::is_archived(self.pg()?, community_id, pubkey).await
    }
    ...
    pub async fn archive(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::archived_identities::archive(
                pool,
                community_id,
                pubkey,
                consent_path,
    ...
    pub async fn unarchive(&self, community_id: CommunityId, pubkey: &str) -> Result<bool> {
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::archived_identities::unarchive(pool, community_id, pubkey).await;
        }
        archived_identities::unarchive(self.pg()?, community_id, pubkey).await
    }
    ...
    pub async fn list_archived(
        ...
        if let DbBackend::Sqlite { pool } = &self.backend {
            return sqlite::archived_identities::list_archived(pool, community_id).await;
        }
        archived_identities::list_archived(self.pg()?, community_id).await
    }
    ...
    pub async fn replace_addressable_event(
        ...
        if let Err(e) = crate::insert_mentions(self.pg()?, community_id, event, channel_id).await {
            tracing::warn!(event_id = %event.id, "Failed to insert mentions: {e}");
        }
    ...
    pub async fn publish_nip43_membership_locked(
        ...
        if let Err(e) = crate::insert_mentions(self.pg()?, community_id, &event, None).await {
            tracing::warn!(event_id = %event.id, "Failed to insert mentions: {e}");
        }
...
mod tests {
    ...
    async fn nip_rs_replacement_hard_deletes_payload_and_watermark_rejects_replay() {
        ...
        let community = CommunityId::from_uuid(make_community(db.pg().unwrap()).await);
    ...
    async fn nip_rs_hard_delete_fence_fails_closed_and_scopes_opt_in_to_transaction() {
        ...
        let mut old_writer = db.pg().unwrap().begin().await.expect("begin old-writer tx");
    ...
    async fn database_guard_covers_legacy_writer_and_nip09_deletion() {
        ...
        legacy_insert(db.pg().unwrap(), community, &a, &d_tag)
            .await
            .expect("legacy insert A");
        let duplicate = legacy_insert(db.pg().unwrap(), community, &a, &d_tag)
            .await
        ...
        legacy_insert(db.pg().unwrap(), community, &b, &d_tag)
            .await
            .expect("legacy insert B");
        let duplicate_b = legacy_insert(db.pg().unwrap(), community, &b, &d_tag)
            .await
        ...
        let delete_pool = db.pg().unwrap().clone();
        ...
        let replay_while_delete_waits = legacy_insert(db.pg().unwrap(), community, &c, &d_tag)
            .await
        ...
        let replay_c = legacy_insert(db.pg().unwrap(), community, &c, &d_tag)
            .await
        ...
        let replay = legacy_insert(db.pg().unwrap(), community, &x, &d_tag).await;
        assert!(
            replay.is_err(),
    ...
    async fn list_communities_owned_by_returns_only_owner_rows() {
        let db = setup_db().await;
        let community_a = CommunityId::from_uuid(make_community(db.pg().unwrap()).await);
        let community_b = CommunityId::from_uuid(make_community(db.pg().unwrap()).await);
        let community_c = CommunityId::from_uuid(make_community(db.pg().unwrap()).await);
    ...
    async fn communities_of_channels_present_for_existing_absent_for_missing() {
        ...
        insert_channel(db.pg().unwrap(), community, existing).await;
    ...
    async fn read_falls_back_to_writer_when_no_replica_configured() {
        ...
        assert!(
            std::ptr::eq(db.pg_read().unwrap(), db.pg().unwrap()),
    ...
    async fn armed_pool_rejects_old_channel_inserts_through_public_api() {
        ...
        let err = event::insert_event(db.pg().unwrap(), cid, &old, Some(channel))
            .await
        ...
        event::insert_event(db.pg().unwrap(), cid, &fresh, Some(channel))
            .await
